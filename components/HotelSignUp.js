...
export default function HotelSignUp({ navigation }) {
  ...
  const [hotelImageUri, setHotelImageUri] = useState(null);
  ...
  const handleUploadHotelImage = async () => {
    ...
    if (!result.cancelled) {
      setHotelImageUri(result.uri);
    }
  };

  const handleUploadHotelFssaiImage = async () => {
    let result = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: ImagePicker.MediaTypeOptions.Images,
      quality: 1,
    });
  ...
  const uploadImage = async (imageUri, imageType, userId) => {
    ...
    let filepath = "users/hotels/" + userId + "/";
    ...
    try {
      await myAuthentication
        ...
        .then(() => {
          ...
          uploadImage(hotelImageUri, "hotel", myAuthentication.currentUser.uid);
          uploadImage(
  ...
  return (
    <View>
      <Text style={styles.selectedTypeHeading}>Hotel Registration</Text>
        ...
        <View>
          {!hotelImageUri && (
            ...
            >
              ...
              />
              <TouchableOpacity onPress={handleUploadHotelImage}>
                <Text
                  style={{
          ...
          {hotelImageUri && (
            ...
            >
              <Image style={styles.icon} source={{ uri: hotelImageUri }} />
              <TouchableOpacity onPress={handleUploadHotelImage}>
                <Text
                  style={{
                    color: Colors.primary,
        ...
        <View>
          {!fssaiImageUri && (
            ...
            >
              ...
              />
              <TouchableOpacity onPress={handleUploadHotelFssaiImage}>
                <Text
                  style={{
                    color: Colors.primary,
                    fontFamily: "serif",
